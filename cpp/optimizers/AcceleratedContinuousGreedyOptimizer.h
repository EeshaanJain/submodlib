...
class AcceleratedContinuousGreedyOptimizer
{
    public:
        ...
        std::vector<double> maximize_fractional(SetFunction &f_obj, std::unordered_set<std::unordered_set<ll>, hash_function> I, float epsilon=0.1);
        ...
        std::unordered_set<ll> maximize(SetFunction &f_obj, std::unordered_set<std::unordered_set<ll>, hash_function> I, float epsilon=0.1, bool verbose=false, bool showProgress=true, const std::vector<float>& costs=std::vector<float>());
};
